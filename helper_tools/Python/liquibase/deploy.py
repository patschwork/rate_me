...
liquibasePropertiesFile = Config.get("liquibase", "liquibasePropertiesFile")
liquibasePromptForDBPassword = Config.get("liquibase", "liquibasePromptForDBPassword")
...
if liquibaseActionValue != "":
    ...
    try:
        try:
            output = subprocess.check_output([liquibasePathExe, "--password=" + password, "--defaultsFile=" + getFilePathRelativeScriptPath(liquibasePropertiesFile), "--driver=" + liquibaseDriver, "--changeLogFile=" + getFilePathRelativeScriptPath(liquibaseChangeLogFile), "--url=" + liquibaseDriverUrlprefix , liquibaseAction, liquibaseActionValue], stderr=subprocess.STDOUT).decode()
        except:
    ...
    try:
        try:
            output = subprocess.check_output([liquibasePathExe, "--password=" + password, "--defaultsFile=" + getFilePathRelativeScriptPath(liquibasePropertiesFile), "--driver=" + liquibaseDriver, "--changeLogFile=" + getFilePathRelativeScriptPath(liquibaseChangeLogFile), "--url=" + liquibaseDriverUrlprefix , liquibaseAction], stderr=subprocess.STDOUT).decode()
        except:
            output = subprocess.check_output([liquibasePathExe, "--password=" + password, "--defaultsFile=" + getFilePathRelativeScriptPath(liquibasePropertiesFile), "--driver=" + liquibaseDriver, "--changeLogFile=" + getFilePathRelativeScriptPath(liquibaseChangeLogFile), "--url=" + liquibaseDriverUrlprefix , liquibaseAction], stderr=subprocess.STDOUT)
        success = True 
    except subprocess.CalledProcessError as e:
        try:
            output = e.output.decode()
